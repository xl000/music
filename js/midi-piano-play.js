class MIDIPianoPlayer {
    constructor() {
        ...
        this.rainDefaultSpeed = 3;   // 钢琴雨默认速度，默认3
    ...
    createSettingsPanel() {
        ...
        settingsPanel.className = 'piano-settings-panel';
        ...
        // 将设置面板插入到控制栏之前
        this.fullscreenPianoControls.parentNode.insertBefore(settingsPanel, this.fullscreenPianoControls);
    }
    ...
    async playPianoNote(noteName, duration = 0.8, startTime = null, withPedal = false, velocity = 1) {
        try {
            if (typeof playNote !== 'undefined') {
                ...
                if (withPedal) {
                    actualDuration = Math.max(actualDuration * 1.5, 1.0);
                }

                // 这里需要修改playNote函数以支持velocity参数
                // 如果playNote不支持velocity，可以忽略这个参数
                await playNote(noteName, actualDuration, startTime);

                this.highlightKey(noteName, actualDuration, withPedal);
            } else {
    ...
    // 高亮琴键
    highlightKey(noteName, duration, withPedal) {
    ...
    createRainEffect(noteName, duration, note) {
        ...
        // 使用统一的移动速度，不再与音符持续时间关联
        const moveSpeed = this.rainDefaultSpeed * 100; // 转换为像素/秒
        
        // 修复：从琴键上边界开始（初始位置），确保下边界贴着琴键上边界
        const startTop = keyRect.top - containerRect.top;
        
        // 初始状态：高度为0，下边界贴着琴键上边界
        ...
        const rainData = {
            ...
            moveSpeed: moveSpeed, // 统一的移动速度
            startTop: startTop, // 琴键上边界的位置
            containerHeight: containerRect.height,
            keyRect: keyRect,
            containerRect: containerRect,
            isGrowing: true, // 标记是否在增长阶段
            currentHeight: 0
        };
    ...
    createTrackSelection(tracksToUse) {
        ...
        tracksToUse.forEach((track, index) => {
            this.fullscreenTrackSelection.appendChild(this.createTrackCheckbox(track, index));
        });
    }

    // 创建音轨复选框
    createTrackCheckbox(track, index) {
        const trackCheckbox = document.createElement('div');
        trackCheckbox.className = 'track-checkbox';

        const checkbox = document.createElement('input');
        checkbox.type = 'checkbox';
        checkbox.id = `fullscreen-track${index}`;
        checkbox.value = index;
        checkbox.checked = true;

        const notesWithPedal = track.notes.filter(note => note.withPedal).length;
        const pedalInfo = notesWithPedal > 0 ? ` (${notesWithPedal}个音符使用踏板)` : '';

        const label = document.createElement('label');
        label.htmlFor = `fullscreen-track${index}`;
        label.textContent = `${track.name} (${track.instrument.name}) - ${track.notes.length}个音符${pedalInfo}`;

        checkbox.addEventListener('change', () => {
            this.updateSelectedTracks();
        });

        trackCheckbox.appendChild(checkbox);
        trackCheckbox.appendChild(label);
    ...
    updateSelectedTracks() {
        this.selectedTracks = [];
        const checkboxes = this.fullscreenTrackSelection.querySelectorAll('input[type="checkbox"]');
    ...
    adjustLayoutForPlayback(isPlaying) {
        if (isPlaying) {
            ...
            this.fullscreenTrackSelection.style.display = 'flex';
